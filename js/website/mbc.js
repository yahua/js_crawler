function mbc_crawler(websiteUrl, html) {
    ...
    var xPathResult = document.evaluate("//div[starts-with(@class,'external-media')]/iframe", el, null,
        XPathResult.ORDERED_NODE_ITERATOR_TYPE, null);
    if (xPathResult){
        var node = xPathResult.iterateNext();
        while(node) {
            ...
            if (iframeUrl && iframeUrl.length>0 && !getUrlInfo(iframeUrl)['scheme']) {
                ...
                iframeUrl = 'http://www.mbc.net' + iframeUrl;
            }
            if (iframeUrl && iframeUrl.length>0 ) {
                var iframeHtml = getHtmlWithUrl(iframeUrl, {'referer':websiteUrl})
                el.innerHTML = iframeHtml;
                var xPathResult = document.evaluate("//div[starts-with(@class, 'external-media')]/a", el, null,
                    XPathResult.ORDERED_NODE_ITERATOR_TYPE, null);
                var m3u8URL = null;
                if (xPathResult) {
                    var node = xPathResult.iterateNext();
                    m3u8URL = getNodeAttribute(node, 'm3u8Link');
                }
                if (m3u8URL) {
